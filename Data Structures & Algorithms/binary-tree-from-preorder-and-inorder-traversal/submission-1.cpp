...
public:
    ...
    TreeNode* helper(vector<int>& preorder, int pre_l, int pre_r, int in_l, int in_r){
        if(pre_l > pre_r) return nullptr;
        int root_val = preorder[pre_l];
        TreeNode* root = new TreeNode(root_val);
        int root_ind = inorder_ind[root_val];
        int l_size = root_ind - in_l;
        root->left = helper(preorder, pre_l+1, pre_l+l_size, in_l, root_ind-1);
        root->right = helper(preorder, pre_l+l_size+1, pre_r, root_ind+1, in_r);
        return root;
    }
};
